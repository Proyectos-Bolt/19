...
function App() {
  ...
  const calculateFare = useCallback((distanceKm: number, waitingMinutes: number, sorianaBonus: boolean = false) => {
    ...
    const baseFareToUse = selectedTripType.fixedPrice || RATES.baseFare;

    // Si es Cristo Rey con subtrip
    if (selectedTripType.id === 'cristoRey' && selectedSubTrip) {
      const basePrice = selectedSubTrip.fixedPrice;
      ...
      const tripTypeExtraFee = (distanceKm >= 3.7) ? 5 : 0;
      const waitingCost = waitingMinutes * RATES.waitingRate;
      return costoAcumuladoParadas + baseCost + waitingCost + sorianaExtraFee + petExtraFee + servicioEspecialFee + tripTypeExtraFee;
    }
    ...
    return costoAcumuladoParadas + fare + (waitingMinutes * RATES.waitingRate) + sorianaExtraFee + petExtraFee + servicioEspecialFee + tripTypeExtraFee;
  }, [selectedTripType, selectedSubTrip, petConfig, servicioEspecialConfig, costoAcumuladoParadas]);
                  ...
                  <div className="border-t border-gray-600 pt-3 mt-3">
                    <div className="space-y-2 mb-2">
                      {(() => {
                        ...
                        const waitingCost = waitingMinutes * RATES.waitingRate;
                        const sorianaBonus = (lastTripSummary.isSorianaActive && lastTripSummary.distance >= 3.7) ? 5 : 0;
                        ...
                        if (selectedTripType.id === 'cristoRey' && selectedSubTrip) {
                          ...
                          if (lastTripSummary.distance > baseDistance) {
                            if (lastTripSummary.distance > 3.5) {
                              baseCost += 20;
                            } else if (lastTripSummary.distance > 2.0) {
                              baseCost += 10;
                            }
                          }

                          if (lastTripSummary.distance >= 3.7) {
                            tripTypeExtraFee = 5;
                          }
                        } else {
                          baseCost = baseFareToUse;
                        ...
                        return (
                          <>
                            ...
                            {sorianaBonus > 0 && (
                              <div className="flex justify-between items-center text-sm">
                                <span className="text-gray-300">🏪 Bonus Soriana:</span>
                                <span className="text-white font-semibold">${sorianaBonus} MXN</span>
                              </div>
                            )}
                            {petCost > 0 && (
                              <div className="flex justify-between items-center text-sm">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-2xl text-green-400">
                          ${(() => {
                            ...
                            const totalFinal = lastTripSummary.costoParadas + distanceCost + waitingCost + sorianaBonus + petCost + servicioEspecialCost + tripTypeExtraFee;
                ...
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-sm text-gray-300">
                      {serviciosExtrasActive || petConfig.active || servicioEspecialConfig.active
                        ? [
          ...
          {tripData.isRunning && currentPosition && (
            <div className="mt-4 bg-gradient-to-r from-purple-800 to-purple-900 text-white p-3 rounded-xl text-center border border-purple-500 shadow-lg">
              <div className="text-xs">
                ...
                <div>Paradas acumuladas: {numeroParadas} (${costoAcumuladoParadas} MXN)</div>
                ...
                <div>Servicio especial: {servicioEspecialConfig.active ? `+$${servicioEspecialConfig.cost} (${servicioEspecialConfig.type === 'recoger' ? 'recoger' : 'comprar'})` : 'Inactivo'}</div>
